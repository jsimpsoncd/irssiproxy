                             ...
                             const char *nick, const char *address)
{
        ...
        /* send the data to clients if it matches whitelist.. */
                
                splitmsg = g_strsplit(event, " ", -1);
				splitargs = g_strsplit(args, " ", -1);
                
                if(strcmp(splitmsg[1], "privmsg") == 0 || strcmp(splitmsg[1], "notice") == 0 || strcmp(splitmsg[1], "join") == 0 || strcmp(splitmsg[1], "part") == 0 || strcmp(splitmsg[1], "kick") == 0 || strcmp(splitmsg[1], "mode") == 0 || strcmp(splitmsg[1], "topic") == 0){
                        
						


                        if(splitargs[0] && *splitargs[0] != '#') {
								
                                proxy_outdata_all(server, "%s", next_line->str);
                        }
                        else if(splitargs[0]) {
						
                                channels = g_strsplit(settings_get_str("irssiproxy_channels"), " ", -1);
				if(channels) {
					char **c;
					for(c = channels; *c != NULL; ++c) {
						if(strcmp(splitargs[0], *c) == 0 ) {
							proxy_outdata_all(server, "%s", next_line->str);
						}
					}
                                }
